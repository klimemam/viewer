...
// The carrier was not released before its typed layer/layout contract was
// complete. NPZ and stream are two encodings of that one contract, so writers
// name generation 3 unconditionally and current readers accept exactly 3.
// Protocol 15 is a separate question: whether a remote wire can carry the
// already-validated typed axes.
inline constexpr int CARRIER_VERSION = 3;

inline bool acceptsCarrierVersion(int version) {
    return version == CARRIER_VERSION;
}

inline std::string versionError(int version) {
    if (acceptsCarrierVersion(version)) return {};
    return "this stream is version " + std::to_string(version) +
           " and this viewer reads exactly " + std::to_string(CARRIER_VERSION);
}

// Parse the complete first line. atoi/sscanf accept `VIEWERSTREAM 3 junk`,
// which turns a malformed declaration into the current contract. Both stream
// readers use this helper so local and peer doors reject the same bytes.
inline std::string parseVersionLine(const std::string& line, int& version) {
    static const std::string prefix = "VIEWERSTREAM ";
    ...
    int64_t parsed = 0;
    for (char c : digits) {
        if (c < '0' || c > '9' || parsed > (2147483647 - (c - '0')) / 10)
            return "not a viewer stream: first line is \"" + line.substr(0, 60) + "\"";
...
struct TreeNode {
    ...
    int parent = -1;
    // Empty is the layer's canonical order.  The only transposed forms are
...
inline std::string scanHeader(std::istream& f, Scan& out) {
    ...
    const std::string versionErr = parseVersionLine(line, out.version);
