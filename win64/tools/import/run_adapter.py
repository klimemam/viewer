...
#: The stream and NPZ carriers are the same schema in two encodings. Their typed
#: contract was completed before release at generation 3; one constant prevents
#: the two writer paths from drifting.
CARRIER_VERSION = 3
...
def stream_out(fh, nodes):
    ...
    """
    _stream_line(fh, "VIEWERSTREAM %d" % CARRIER_VERSION)
...
def write_npz(path, nodes):
    ...
    out = {"__viewer": np.array(CARRIER_VERSION, dtype="int32"),
