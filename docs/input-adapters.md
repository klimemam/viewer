...
| # | 内容 |
|---|---|
| 1 | `__viewer <version>` を仕様に入れ、読む側が版で分岐できるようにする |
| 2 | viewer 側の**読み手** (adapter 経由でなく直接開く) |
| 3 | viewer 側の**書き手** (`Save as viewer npz...`)。stack/series をそのまま保存 |
| 4 | 往復の検査 — 書いて読んで、層・軸・単位・CFA・note が同一であること |
| 5 | 版を上げるときの規則 (追加は互換、意味の変更は版を上げる) |
