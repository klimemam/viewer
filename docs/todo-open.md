...
## 台帳 (2026-07-30 更新 — 各項目の現在地)

**閉じた** (main 統合・push 済み):
- 項目0 stack派生 (c72fcdb) / 項目1+26 Preview・ダブルクリック (ce02f12) /
  項目3 サイズ違いFit=大きい方 (ecbe706) / 項目4 `..`行 (d359073+3df373d) /
  項目10+A5 remote自動再接続 (6683352) / 項目17 インスタンス化 (6683352) /
  項目21+A4 stale淡色+TSVに(stale)語 (35d31d8) / 項目27 CFAサンプリング (c12b623) /
  項目19の実欠陥5件 (ecbe706) / 項目25 Bデフォルト+Shift+C (6047aab+5419de1)
- 項目12 はほぼ閉: パスバー脱箱化・places格納・ツールバー1行化 (ce02f12)、
  接続バンドのインスタンス別化 (6683352)。残るのは filter/search 統合 (=項目14) のみ
- TODO外の本日分: SNRカーブ / Temporal書出再設計+カスタムX軸+ズーム+軸貸与 /
  比較スロット(数値N+タイル+Temporal) / 画像書出(dot-by-dot+モンタージュ) ほか
...
- 項目28 別ウィンドウで開く — インスタンス別autosave(ロック+PID生存判定+stale回収)、
  prefs は副窓で読み取り専用、カスケード配置、ssh二重接続のトースト明示。
  項目21残(Cの値域)/項目19系メニュー簡素化/ESC離脱/ペイン2行/フレームリニアリティ
  (rev4-style選択付き)/モンタージュper-frameレンジも同日閉

**開いている** (判断待ち):
- 項目16 平坦統計 (残: B2 σ表記 / B4 detrend既定 / B5 置き場所。B1+B3 は回答済み)
- 項目14 filter/search の入口 / 項目5 用語 (C1) / 項目15 表記 (C2) /
  項目8 zoom% (C3) / 項目6 検索の複数選択 (C4含む) / 項目7 検索進捗
- 設計文書レビュー: docs/compare-n.md / docs/export-design.md

**開いている** (その他):
- 項目11 Inspectorのremote行 / 項目13 FilesとBrowseのUI不一致(スクラブ2重) /
  項目18 表示どおりコピー / 項目22 パネル追加ボタン / 項目23 数値書式 /
  項目24 残渣 / 未着手機能 (DSNU/PRNU・欠陥画素・blink/RTS・DR・暗電流・
  OpenEXR・動画・ベタRAWリモート・params_schema) / 共有測定カーネル+parity
...
> **更新 (2026-08-09, #107):** 後者は**削除済み**。選択から σ_t を計算する経路は
> もう無く、本項をやるときも**復活させない** —— σ_t は stack の属性なので、
> 選択に対する正しい σ_t が存在しない (docs/stats-taxonomy.md §3.1)。
> 検索結果に載せるべき選択動詞は「stack として開く」「stack 毎に frame average」
> の 2 つで、時間統計は開いた stack か連番行から測る。
...
## 10. セッション復元で remote (ssh) が戻らない — 済 (browse-instances, 決定A5)

### 状態更新 (2026-07-30): `rbplace <num> <url>` で実装、自動再接続

ユーザー決定 A5 = **auto-reconnect** (click-to-reconnect ではなく)。セッションに
インスタンスごと1行 `rbplace <num> <url>` (placeUrl 経由なのでポートも運ぶ)。
復元時は各インスタンスが**自分のワーカーで非同期に**繋ぎ直す — UI は ssh を
待たない。失敗はそのインスタンスのインラインのエラー帯に出る (グローバルな
ダイアログではない)。キー追加のみなので旧ビューアでも壊れない。
`--browse-keys-selftest` の `sessrt` が保存→全撤去→復元→両インスタンス再接続を
毎回検証する。以下は経緯の記録。
...
> **ユーザー追記 (2026-07-30)**: 「今の認知負荷がとにかく高い．パスを四角の連続で
> 表示するとか，実装上の省力化，みたいなところが良い結果になってない．」
> — 診断そのもの: 上部が今の形なのは **ImGui の部品を積むのが実装として一番安い**
> からで、誰かがそう選んだからではない。パンくずが SmallButton の連鎖 (=箱の列) に
> なっているのが named example。
> **表示レベルの整理 (パスバーの脱・箱化、places のパスバー内格納、ツールバー1行化)
> は項目17 を待たずに先行着手** (2026-07-30、preview-fixes2 エージェントに統合仕様
> として渡し済み)。接続バンドの吸収だけが 17 待ちで残る。
> **追記 (browse-instances)**: 項目17 の実装で接続バンド ("more" の中の
> peer 表示 / disconnect / 検索行) は**インスタンスごと**になった。場所中心の
> トップバーへの**吸収**そのものは未着手のまま — インスタンス化が要求する以上の
> 再設計はしない、の線引きどおり。項目14 (filter と search) も未統合のまま。
...
## 17. Browse パネルは分けるべきか — そして「複数あってよいか」【決定・実装済】

### 状態更新 (2026-07-30): 「インスタンス化」で決着、branch `browse-instances` で実装

ユーザーの選択どおり **instance-able views**: Browse は singleton をやめ、
**1インスタンス = 1つの場所を見るビュー** (`App::BrowseInstance`) になった。
下に数えたコストは全額支払い済み — App::rbrowse・drawPanelRemote の関数ローカル
static 約21個・g_rb* グローバル (cursor/sort/toolbar + nav 追加分の history 4個 +
deferred キュー) は全部インスタンスへ移動。セッション規律は「one Session, one
owning thread」を**インスタンスごとに1ワーカー+1セッション**へ一般化 (多重化は
しない: 遅い ssh connect が他インスタンスの listing を直列に塞ぐのを避けた)。
ウィンドウ ID は初代が `Browse###Remote` を保持 (レイアウト無傷)、2枚目以降は
`Browse N###BrowseN` (番号はセッションに保存され安定)。「+」とView > New Browse
Panel が増設、閉じたら破棄、**最後の1枚は非表示になるだけ** (常に開き直せる)。
矢印キーはフォーカスされたインスタンスが持つ。preview スロットは全インスタンス
共有のまま (正典どおり)。`--browse-keys-selftest` は 227 アクション (2インスタンス
同時に別ディレクトリ / filter・cursor・選択・history の独立 / フォーカス追従 /
閉じる・開き直す / セッション往復) 。ローカル/リモートはパネルの種類ではなく
**場所**のまま — browse-as-file-manager.md の裁定2つは実装でも生存。
...
> **スケジュール確定 (2026-07-30、ユーザー指示)**: Watch は**週末に、参照設計
> (frame の共有参照化 — 項目0の派生 stack を「コピー」から「参照」へ置き換える
> 作業) と一緒に**やる。先にやらない。
>
> 理由はこの2つが同じ土台を触るから: 参照なら「元ファイルの更新」は
> それを参照する全 stack に一度に伝わり、Watch の更新伝播は自明になる。
> コピーのまま Watch を作ると「どのコピーが古いか」の追跡機構を作って
> 週末に捨てることになる。
>
> 既に決まっている仕様 (A6/A6b、2026-07-30): 既定は**通知→手動 Reload**
