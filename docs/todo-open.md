...
## 27. Histogram と ROI 統計にも CFA サンプリングの偏りがある【測定の正しさ】

Projection 側は `47eec0c` で修正済み (ストライドが常に 2 以上になり、
RGGB で **B プレーンが無サンプルになって行ごと消えていた**)。
**同じ家族の偏りがあと 2 箇所に残っている。失敗の形が違うので別対応とした。**

### 仕組み (Projection とは別の壊れ方)

どちらも **1 次元の通し番号 `p` をストライド**している:

- `recomputeHistogramIfNeeded` (main.cpp ~8892)
  `size_t step = std::max<size_t>(1, total / 1000000);` → `for (p = 0; p < total; p += step)`、
  `x = rx + p % rw`, `y = ry + p / rw`
- `roiBasicStatsUncached` (main.cpp ~12471)
  同じ形で `total / 200000` (ROI ごとの上限が低いので、**もっと早く発火する**)

`step` が**偶数**で `rw` も**偶数**のとき、`p` は常に偶数なので
`x = p % rw` も常に偶数になる。つまり **奇数列の画素が 1 つも取られない**。
RGGB なら R (偶x偶y) と Gb (偶x奇y) だけが残り、**Gr と B は n=0**。

センサの幅はほぼ常に偶数なので、**`step` が偶数になった瞬間に必ず起きる**。
報告されている発火点: Histogram は 1 Mpx 超 (1920x1080, 2048x2048, 4096x3072)、
ROI 統計は 800x600 から。**未検算 — まず再現させること。**

### なぜ重いか

この 2 つが養っているのは、**人が数値として読む場所**:

- Inspector の Statistics 表 (mean / std / var、プレーン別)
- ROI パネルの表、そして **Copy table (TSV)** でそのまま資料に行く
- Histogram のプレーン別カーブ (Gr と B が空に見える)

数値が**出ていない**のならまだ気付くが、この形は
**残った 2 プレーンの値だけが普通に表示される**ので、壊れていることが見えない。

### 直し方 (Projection で採った形をそのまま使える)

通し番号をやめて、**モザイクセル単位で 2 次元に歩く**:
セルの幅 (Bayer=2, Quad=4) で x,y を進め、**着地したセルの全画素を取る**。
これならストライドがいくつでも各プレーンのサンプル数が等しい。
サンプル上限はセル分を掛けて調整すること (Projection と同じ)。

### 検査 (ここを外すと意味がない)

**旧動作で落ちることを先に確かめること。** Projection の V17 で実際にやった:
最初に書いた検査は、統計が無効なプレーンを `continue` で飛ばしていたため
**バグが空にするまさにそのプレーンを見なかった**。
壊れたケースを跨ぐ検査は検査になっていない。

assert すべきは「**全プレーンの n が 0 でなく、かつ互いに近い**」。
フィクスチャは `step` が偶数になる大きさが要る (Histogram なら >1 Mpx)。
`tools/gen_testdata.py` に大きめの Bayer を 1 枚足すことになるはず。
