...
- スロットの上限 (無制限か、実用上 4〜8 か)。キャッシュはスロットごとに
  ヒストグラム・プロファイル・Temporal を持つので**メモリと計算が線形に増える**。
  今でも B のキャッシュは実測でミリ秒単位のコスト (`abStepBusyUntil` の
  スロットリングはそのために入っている)。
- スロットの**指定方法**。今は「B にする」の1操作。N なら
  Files の行に「スロットに入れる」→ 空きスロット、あるいは番号指定。
- 命名。A/B/C… か、1/2/3 か。**series の member と混同されない**こと。
- セッション保存 (今は `compareB` 1つ)。
