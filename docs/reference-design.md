...
答え: **半分だけ包含する。walk の変更は不要、消費者と持続形が欠けている。**
3つに分けると:

### 10.1 ライブセッション内 — 鍵はあるが、読者がいない

stackavg の平均 doc (`0e4d4ec`) は source を**共有しない** — path は意図的に
空で、レジストリの外にいる (§6.2 の tuple を持たないから、これは正しい)。
だから §3.2 の walk は平均の画素に**届かない**し、届くべきでもない (平均は
membership ではなく計算結果)。walk が保証するのは `si->stackRev++` まで。

穴は、平均がこの鍵を**消費していない**こと。追補:

- 平均 doc は生成時に `(seqId, 生成時の stackRev)` を記録する。
- 描画/測定経路は `seqInfo(seqId)->stackRev` と比較し、不一致なら doc に
  **stale マーク**を立てる — 絵は勝手に描き直さない。平均は報告書に載る絵
  なので、無言の再計算は無言の上書きと同罪。再計算はユーザーの一手
  (Recompute) で、実行時に記録を更新する。

### 10.2 セッション復元 — #82 の核心。世代は tuple で言う、二つ目の機構は作らない

`stackRev` はセッションを跨がない。跨ぐ同一性語彙は**既にある** — レジストリ
の identity tuple (path + mtime + fsize、§6.2)。#82 のメモが提案する
「path+世代」キーは、**この tuple そのもの**である。よって:

- `stackavg` レシピは対象 stack の**入力 tuple の記録** (各メンバーの
  mtime/fsize、または全メンバーの digest) を一緒に保存する。
- 復元時は今日と同じく再計算する。ただし記録した tuple と実際に読んだ
  tuple を比較し、不一致なら note と Files 行で言う:
  「recomputed from newer files (source changed since …)」。
- 「同じ絵か」は画素比較ではなく tuple 比較で判定する — §6.2 と同じ判定
  基準の再利用であり、世代概念を二重に発明しない。

#82 の残る3問はこの語彙で自然に決まる: (1) 同一セッション内で2回平均する
と doc は2つできる — レジストリは source を dedup するのであって計算結果は
dedup しない。2つが同じ入力 tuple を記録していれば「同じ絵」とツールチップ
で言える。(2) セッションの2行は各自復元・各自再計算 — 潰れない。世代が
違えば各自がそう言う。(3) stack の batch 移動はパスを変えないのでレシピは
追随する。

### 10.3 #56 (混合世代 stack の永続マーカー) との統合 — マークは1機構

「部分 reload 後の混合世代 stack」(レビュー所見 [24] = #56) と「復元で別世代
から再計算された平均」は同じ一文の事実:

> この doc の画素は、記録された入力 tuple と今のディスクが一致しない状態で
> 作られた / 残っている。

だからマークは1機構でよい: doc 単位の generation note (provenance 欄) +
