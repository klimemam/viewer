...
**[compare-n.md](compare-n.md) との関係。** あちらは指針 + 着地記録 (§11 に増分が
積まれる生きた台帳) で、本書はそれを**置き換えない**。本書は #60 が板に載せた
要約「スロット列・A=カーソル・上限A+6」を6つの問いに割り、**実物を数えて**
答える正規仕様である。compare-n.md の判断はほぼ全て継承し、**改定は1点だけ**:
上限 A+B+6=8 (compare-n.md §7, §8-3) を **A+6=7 に切り下げる** (本書 §4、
数えた根拠つき)。今後、着地の事実は compare-n.md §11 に、仕様の変更は本書に書く。
衝突したら本書が勝つ。
...
比較とは**1本の順序付きスロット列**である。A はカーソル (席ではない)、
B 以降は「stack への参照 + follow 規約」の**席**で、各席は A のフレーム番号に
追従し、追従できなければ [stale] と言う。画像側の顔は **laid out (タイル =
row/grid)** と **collapsed (diff / N-blink)** の2系統だけ、wipe / diff は厳密に
A 対 B のまま、別の文字と比べたければ**昇格**が唯一の操作。数値側は全パネルが
スロット添字のキャッシュで N に伸びる。**上限は A+6 (総面数7) で、縛りは色**
(`slotInk` の7本) である — 幅でも表でも RAM でもない。

## 1. 問い1: 今どこまで N になっているか — 台帳

**数え方。** (a) 各パネル・キャンバスの描画経路を読んだ。(b) スロット機構の
消費者を grep した: `cmpExtra|resolveSlots()|compareExtras()` は 18 ファイル
104 箇所 (製品コード 55 / selftest 49)。(c) B 席の読み手を grep した:
`cmpB()|resolveB()|compareBUid` は 20 ファイル 101 箇所 (製品コード 44 /
selftest 57)。(c) の 44 が §6 D3 (B の一本化) の移行量である。
...
**要約: 数値側とセッションはほぼ N 済み。残る本体は (i) 状態の一本化 (B と
bool)、(ii) 画像側の laid out のモード化、(iii) Projection 曲線 —— そして
本書が足す (iv) 上限の切り下げ、である。**
