...
| Histogram プロット | overlay: 1面に絞れば全席重ね、上限 `slotInkCount()` + 打ち切り文字の名指し。side-by-side: 1側1ペイン (§7.1) | `core/ui/panel_histogram.inc` |
| Projection **曲線** | 同上 — 面セレクタは `histPlane` 共有 (§7.2)、軸不一致は全体が side-by-side に退いて名指し | `core/ui/panel_projection.inc` (§6 D5) |
| Auto の解決 | 画像の並びに従う (split → 並べる / collapsed → 重ねる)。blink 中は表示側を太線 | `abSideBySide` / `abEmphSide` (`compare.inc:123-`) |
...
### D5 — Projection の N 重ね (= compare-n.md §12 I7) 【済 (裁定 2026-08-14)】

中身は I7 のとおり: 面セレクタは `histPlane` 共有 (§7.2)、1面のとき `slotInk`
重ね、`slotLegendRow`、軸一致は A 対各席 (`abProjOverlayable`)、上限は
`slotInkCount()`、はみ出した席は**名指し**。ヒストグラムと**同じ2つの上限**
(1面であること・インクが足りること) を**同じ文法**で言う。

**I7 からの1点の精緻化 —— 軸不一致の答え。** I7 は「不一致席は描かず・
伸ばさず・名指し」だったが、これは side-by-side が A|B の2枚しか持てなかった
時期の文である。§7.1 で side-by-side が N ペインになったので、**不一致が
1つでもあれば panel 全体が side-by-side に退く**方が良い —— どの席も自分の軸
の上に描かれ、**曲線を失う側が無い**。伸ばさないという禁は変わらない。名指し
は「どの letter が、どの範囲で A と違うか」を全側の座標つきで出す。

試験 (赤→緑を踏んだ): selftest N6 の固定 `curves == "AB"` を `"ABCDE"` に
**動かした**。導出: C, D, E を armed、全側同一 ROI・同一軸、`histPlane` は
1面 → 5 ≤ `slotInkCount()` = 7 なので曲線は A, B, C, D, E の5本。全面を軸に
出した場合の `curves == "AB"` / `said-no-curve == "CDE"` も N6 に残してあり、
語られる上限がまだ語られていることを固定している。
...
D5 は済 (裁定 2026-08-14)、D6 はいつでも。どの順でも各段単独で緑。
...
9. **Auto は N でも画像の並びに従う** — split は1側1ペイン、collapsed は
   重ね + 表示中の側を太線。**§7.1 (2026-08-14, #60)**。
10. **Projection の面セレクタは `histPlane` 共有。** **§7.2 (2026-08-14, #60)**。

### 7.1 統計パネルの Auto は N で何に解決するか

**決定済み (2026-08-14, #60)。** Auto は **画像の並びに従う**。これを N でも
貫き、**パネルの種類では分けない**。

- 画像が**並べて**表示される場合 (split) → プロットも同じように**並べる**。
  A, B, C なら**3つ並ぶ** —— 「二枚だから半分ずつ」ではなく、**1側 1ペイン**。
- 画像が**重なって**表示される場合 (wipe / blink / diff) → プロットも**重ねる**。
  **ただし表示中のスロットが分かる状態にする (太線等)。**

`abSideBySide()` はもともとこの規則を書いていた (`compare.inc:123`) —— 2で
止まっていたのは**パネル側の side-by-side に「半分が2つ」が打ち込まれていた**
からで、Auto の解決ではない。したがって変えたのは**解決先ではなく実装の幅**で
ある。`abStatsLayout` は1つのグローバル設定のまま (ab-stats-plan §3、本書 R3)。

**overlay の強調**は表の blink 追従 (#76) の**チャート側の対応物**である。表は
「今見ている側」を1つ出し、チャートは全員を出しつつ「今見ている側」を太くする。
どちらも「画像で今見えているものが、数値側でも主役」という**同じ規則**。

強調の適用条件は、**一時に1つしか見えていない collapsed** —— つまり blink
(`blinkSideIndex()`) だけである。**wipe と diff は両側が同じ瞬間に画面にある**
ので「今見ている側」が存在せず、主役を発明することは画像がしていない主張を
することになる。side-by-side では各ペインが名前バンドを持つので、重みは
バンドの繰り返しにしかならない —— 強調しない。

実装 (`abEmphSide()` + `abCurveW()`):

- 太さは共有軸上の相対量 (1.2/1.4 px に対して 2.2倍)。**色は letter に使い
  切っている**ので、強調に借りられる次元は重みしかない。
- 強調が効いている間は**全側が線になる** (A の塗りを外す)。塗りと線の間で
  重みは比較できず、塗られた A は主役でなくても主役に読めるため。
- probe は**letter を記録する** (`emph=B;`) —— px 値ではなく構造量。

### 7.2 Projection の plane セレクタ

**決定済み (2026-08-14, #60)。** **ヒストグラムと共有する** —— `histPlane`
1つを両パネルが読み、片方で変えると両方変わる。1面に絞るのは**比較の状態**
であって、パネルの状態ではない。パネルごとに別の面を黙って持つと、同じ比較
画面が2つの異なる対象を映すことになる。

これは D5 (投影の N 重ね) の**着手障害そのもの**だった: 4面 × 5席 × 2軸 =
40本なので、席が軸を共有できるのは**1面のときだけ**であり、投影に「どの面か」
を言う手段が無い間は、そこへ到達する道が存在しなかった。

### 7.3 まだ決まっていない (着手は塞がない)

1. **side-by-side の大 N: 縮む (提案) か横スクロールか。** 2026-08-14 の口頭
   回答は「狭くなる,縮むでよい」。D4 と本実装は「縮む + 最小幅を割ったら
   言って退く」で書いてあり、正式に文書化された裁定が出れば §10 の OR を消す。
2. **機能名「A/B」の改名。** terminology.md の管轄。本書は固有名として存置。
3. **フレームに収まらない ROI: 空欄 (現状) か交差部か。** 採るなら母集団の
   変化をセルに書く必要がある。
4. **blink でも side-by-side でもない collapsed の表は A を出す (現状追認)。**
5. **Inspector の画素読み出しの N 化。** v1 は A/B のまま (席の値は表の仕事)。
