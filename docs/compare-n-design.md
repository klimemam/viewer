...
従来の上限 **A+B+6（合計8面）**を、**A+6（合計7面）**へ変更する。
...
N比較は、**順序を持つ1本の比較対象リスト**として扱う。

- **A**は、現在表示している対象を示すカーソルであり、固定スロットではない。
- **B以降**は、stackへの参照とフレーム追従規則を持つ固定スロットである。
- 各スロットはAのフレーム番号に追従する。対応するフレームがなければ
  `[stale]` と表示する。
- 画像の表示方法は、**laid out**（row / gridのタイル表示）と、
  **collapsed**（diff / N-blinkによる単一表示）の2系統とする。
- wipeとdiffはA対Bの2対象比較を維持する。別のスロットと比較する場合は、
  そのスロットをBへ昇格させる。
- 数値パネルは、スロット番号をキーとするキャッシュを使い、N対象へ対応する。
...
3の製品コード44箇所が、§6 D3「Bをスロット列へ一本化する」で移行対象となる。
