...
> **実装状況 (2026-08-10) — 段階1 が入った。** 本文は仕様のまま (実装が
> 仕様に追いつく、逆ではない)。入ったのは **§2 (版交渉と互換)・§3
> (`version` / `headline`)・§11 (ホスト帳簿との接続)** と、それを載せる
> **§4 の `psAnalyzerV3` / `psAnalyzeSink3` / `psHostApi::register_analyzer3`**
> (#46 段階2 がこれで閉じる)。**まだ無い**のは §5 (`psStack` /
> `psStackAnalyzerV3`)・§6 (`min_frames`)・§7 (series の形と席)・§8
> (`emit_number_u` / `emit_map`)・§9・§10。
> 未実装分は §2.2 の探針規則どおり**予約席**に居る: `psHostApi` の
> `register_analyzer3` の後ろ、`psAnalyzeSink3` の `reserved[8]` の先頭2席。
> **ヘッダに名前を書くのは実装する段階** —— 名前が書かれた瞬間に形の約束が
> 始まるので、席は番地だけ確保して名前は伏せてある (§7.2 が kind 3/4 に
> 対して採った線をそのまま全未実装席に適用した)。どれも**版上げ無しに**
> 埋まる。同梱アナライザの V3 移行 (§1 の機械的作業) も未着手 —— 全 V2 登録
> のまま動いており、`version` 欄は空である。
