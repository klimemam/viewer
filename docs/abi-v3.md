...
> **実装状況 (2026-08-10) — 段階2 が入った。** 本文は仕様のまま (実装が
...
> が入った。運搬は **§9.1 (in-process、常駐フレームへの参照) だけ**が実物で、
> `get_frame` はポインタを返し `release_frame` は no-op である —— pull の口を
> 固定したのは §9.2/§9.3 を後から塞がないためで、その2つはまだ無い。
> **まだ無い**のは §7 (series の形と席)・§8 (`emit_number_u` / `emit_map`)・
> §9.2/§9.3・§10、および §5.2 末尾の住処 (Analysis パネル / Measure メニュー
> への stack アナライザの掲示とグレーアウト) —— 段階2 はホスト側の口までで、
> UI はまだ張っていない。
...
> **版上げ無しに**埋まる。同梱アナライザの V3 移行 (§1 の機械的作業) も
> 未着手 —— 全 V2 登録のまま動いており、`version` 欄は空である。
