...
> **状態: 判断待ち。** [analysis-layers.md](analysis-layers.md) §4 の骨格
> (「方向であって寸法ではない」) に寸法を与える実装仕様。§10 が ABI v3 に
> 預けた4件 — 構造体の確定・stack の運搬・remote MEASURE の拡張方向・
> kind 3/4 の口 — と、判断record 6 が v3 と同時と決めた #46 の version 欄を
> ここで確定する。開いた選択は全部 §13 の判断リストにあり、それ以外の本文は
> 決定文として書く。コードはまだ無い — この文書が先で、実装は判断の後。
...
- **remote のワイヤ形式**: メッセージのバイト配置・版は
  [remote.md](remote.md) の領分。§10 は op の存在と成立条件 (パリティ) の
  方向だけを決める。
 ...
 *   3 - layer-typed analyzers (docs/abi-v3.md): psAnalyzerV3 (frame,
...
- **`abi_version >= 3` が保証するのは §13 判断リスト確定後の v3 中核**
 ...
 * a file - is host business and invisible here (docs/abi-v3.md §9). */
...
[measure-ux.md](measure-ux.md) の既存規則)。
...
推奨は**両立**: 形 (§7.1) は v3 ヘッダで凍結し、`register_series_analyzer3`
...
[input-adapters.md](input-adapters.md) §4.3.1) に従い宣言欄を設ける:
...
- 帳簿に `version` 列が増える (v3 記述子のみ。V1/V2 は空のまま —
  **推測して埋めない**。dll のファイル版リソースも読まない: 宣言では
  ないので)。
- ステータス行 ([measure-ux.md](measure-ux.md) §2): `アナライザ名
...
1. **version の粒度** — (a) 記述子ごとの `const char* version` 欄 /
   (b) dll 単位の第2エクスポート (例 `psPluginVersion()`)。
   **推奨 (a)。** provenance 行は「名前 + 版 + ファイル」で並ぶ — 版は
   名前の隣に住むのが照合の最短距離で、ファイル単位の情報は帳簿が既に
   ABI 外で持っている (段階1の切り分けの裏面)。
2. **stack アクセスモデル** — (a) pull (get_frame + release_frame) /
   (b) 全枚ポインタ配列 / (c) push 型逐次。
   **推奨 (a)。** 全常駐を約束せず・乱択アクセスを保ち・共有メモリと
   peer 側に同形で載るのは pull だけ (§9.4 の却下表)。
3. **部分ロード契約** — (a) min_frames (宣言) と frames/expected (事実) の
   両建て / (b) min_frames のみ / (c) n/N のみ。
   **推奨 (a)。** 宣言は呼ぶ前に裁き、事実は呼ばれた後に語る — (b) は
   provenance の n/N が欠け、(c) は全プラグインに拒否コードを重複させる。
4. **series の口** — (a) 形をヘッダ凍結、登録席は NULL のまま出荷
   (組み込みが張ってから探針で開く) / (b) v3 で口まで開ける / (c) 形も
   書かない。
   **推奨 (a)。** 正典 §10 (組み込みが先) と判断record 6 (版上げを2回に
   しない) を NULL 探針が同時に満たす — (b) は正典と衝突、(c) は次の
   版上げを予約してしまう。
5. **単位の宣言欄 emit_number_u** — (a) 追加 (宣言 > キー名規約 > dtype の
   優先順位) / (b) キー名規約のみ続投。
   **推奨 (a)。** キー名規約の成立理由は「欄が無いから」と文書化済みで、
   新しい口に欄を作れる以上、家訓 (宣言できるなら宣言が勝つ) が勝つ。
6. **emit_map を v3 に入れるか** — (a) 入れる (key / rect / unit /
   float32 / NaN=欠測 の最小欄。意味種別の欄は #49 確定まで置かない) /
   (b) #49 確定後の次版へ送る。
   **推奨 (a)。** stack 解析の主産物 (欠陥マップ・有効枚数マップ) に口が
   無いと版上げがもう1回確定する — 欄は #49 のどの候補も塞がない最小に
   絞ってあり、受け側の型・表示・帰属は #49 の手に残る (正典 §1.3 との
   衝突はこの線引きで回避)。
7. **運搬の3態** — (a) in-process は常駐参照 (コピー0) / #45 ワーカは
   共有メモリ窓 / remote は peer 側実行、で確定 / (b) いずれかを ABI に
   固定する (例: 共有メモリを必須にする)。
   **推奨 (a)。** 画素の居る場所で測るが3面すべてで最小運搬になり、
   ABI は pull の口だけ固定すれば運搬は差し替え可能のまま残る。
8. **remote 拡張** — (a) `MOP_PLUGIN_ANALYZE` を name+version 等値
   パリティで足す方向を採る (不一致は両版併記の理由つき拒否、黙って
   local に振り替えない) / (b) remote のプラグイン実行は当面作らない。
   **推奨 (a) を方向として。** version 欄が生まれて初めて「peer と同じ
   計算機か」が問える — #45 の「パリティ実測不能」反論への構造的回答で、
   ワイヤ形式の細部は remote.md の実装時でよい。
