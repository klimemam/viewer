...
> 本書はその提案を骨格に据えて書き直したもの。同日の後続のやりとりも
> 織り込み済み: **Mean と Sum の併設 (§3.2)**、**`std / mean [%]` への改名
> (§7)**、**「batch 既定・map 型は UI の話」の訂正 (§1.3, 判断2)**。
> 残る判断は §12。個々の推定量の数式・補正・表示の細部はここでは決めない
> (§10 — #57 の領分)。
...
- **Mean と Sum の両方を持つ** (ユーザー裁定 2026-08-06)。当初案は「Sum は
  NaN の画素毎除外の下で正直に定義できない」だったが、指摘のとおり
  **Mean も同罪** — 母集団が画素ごとに違う事実は、正規化で見えなくなるだけで
  消えてはいない。正直さは畳み方の選別ではなく**申告**で担保する:
  どちらも除外数と有効枚数を必ず運ぶ (frame 平均が既にしている申告と同じ)。
...
(経過時間・温度に対する mean の曲線 = §3.2 の cv) に純化する (判断6)。
...
- **手間の吸収は UI の仕事** (ユーザー指摘 2026-08-06: 解析のたびに set を
  ...
  推定値列と同じ規約で、画面に出たものを人が見て確定する (判断2)。
...
| 量 | 種と層 | 名前 | 扱い |
|---|---|---|---|
| ROI の σ/mean (そのまま) | General / frame (ROIs 列) | **`std / mean [%]`** | **改名決定済み** (ユーザー裁定 2026-08-06。旧 `PRNU [σ %]`)。素の名前になり、General から有名名が消えた。実装 (列名・ツールチップ・selftest・manual/analyzers) は本書の外で対応中 |
...
PRNU を名乗る数は plugin の `prnu_pct` 1つと素の名前の比になり、
**改名が main に入れば #66 は閉じられる**。
...
| 時刻・所要 | 既存の出所行のまま |
| 計算者 | viewer 版、または**プラグイン name + version + ファイル** (#46。version は ABI v3 の新設欄 §4、**dll パスはホスト側の帳簿で足りるので ABI を待たない** — 判断5) |
...
| 5 部分ロード n/N | **存続** (新判断4) — 再構成の影響なし。§3.5 のまま |
| 6 provenance 2段 (#46) | **存続** (新判断5) — set が構造化 provenance になる分、内容はむしろ強まった (§8) |
| 7 フレームリニアリティ | **存続** (新判断6) — Series Analysis パネル担当という着地が明確になった (§3.3) |
| 8 正典への3行追記 | **変換・拡大** (新判断7) — 階層挿入と束縛例外を含む正典修正パッケージになった (§11) |
...
いずれも**修正として明示的に**入れる (判断7。黙って混ぜない):
...
各項目3行: 決めること / 推奨 / 選ばなかった場合。理由の全文は本文の節にある。
決定済みは消した: `std / mean [%]` への改名、Mean と Sum の併設 (いずれも
2026-08-06 裁定、§7 と §3.2 に反映済み)。

1. **AnalysisSet のメンバは包含か、参照か。**
   推奨: 参照 (§1.1)。同じ dark stack を PRNU 用と欠陥画素用、両方の set が使うため。
   包含にすると: dark を使い回すたびに stack を開き直すことになる。

2. **set の手間を UI がどう吸収するか。**
   推奨: 1クリック実行はノードを作らず出所欄が中身を運ぶ。ノード化は保存時。同じ batch の2つ目以降の set 作成は、前回の dark/flat を**提案として**事前入力する (提案であって代入ではない — 推定値列と同じ規約 §6)。
   やらないと: 解析のたびに同じ束縛をやり直す — 「いちいち作ってられない」の実害がそのまま残る。

3. **Stack パネルの畳み方の残り。**
   決定済み: Mean と Sum の両方。どちらも除外数と有効枚数を必ず併記 (§3.2)。
   残る推奨: 既定は Mean (σ_fpn と同じ蓄積器)。副表示に per-frame の重ね描き。pooled は保留。Sum には画素ごとの有効枚数マップを添えられるように。

4. **部分ロードの stack を測ってよいか。**
   推奨: 測ってよい。「8/24 枚」の併記を必須 + 解析毎の `min_frames` + ロード中は待つ (§3.5)。
   完全ロード必須にすると: remote の stack と、メモリで切れた大 stack が測れない。

5. **プラグインの出所表記 (#46)。**
   推奨: DLL のパスと名前は今すぐ (ホスト側だけで済む)。version 欄は ABI v3 と同時 (§4)。
   version だけ先行すると: ABI の版上げが2回になる。

6. **フレームリニアリティの最終形。**
   推奨: frame をメンバとする series へ移す (§3.3)。今の暫定実装は移行まで維持。
   現状を最終形にすると: 「stack は同一条件」という定義に例外ができる。

7. **正典 (terminology.md) の修正の承認 (§11)。**
   内容: 階層に AnalysisSet を挿入 / set のメンバは参照 (唯一の例外と明記) / 用語4種 / 不変条件3行。
   否決すると: AnalysisSet は本書ローカルの概念に留まり、Files・セッション・正典が別々の階層を語る。
