...
2. **set はいつ Files のノードになるか。**
   推奨: 保存したときだけ (§6)。1クリック実行は set を作らず、結果の出所欄に中身を書く。
   常にノード化すると: DSNU を3回実行すると Files にノードが3つ増える。

3. **Stack Histogram / Projection / ROIs は時間方向をどう畳むか。**
   推奨: 既定 = 時間平均 (frame 平均と同じ計算 §3.2)。副表示 = per-frame の重ね描き。
   Sum は入れない: NaN を画素ごとに除外すると分母が画素ごとに違い、和は嘘になる。

4. **ROIs の `PRNU [σ %]` 列はこのままでよいか。**
   推奨: このまま (#65 の決定+ツールチップ)。「General に有名名なし」の唯一の例外として正典に明記 (§7)。
   改名するなら: `NU [σ/mean %]` 系 — 一度退けられた名前に戻ることになる。

5. **部分ロードの stack を測ってよいか。**
