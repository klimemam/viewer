...
| A20 | frame-lin パネルの節とスタブ(**引退**: §3.3 の移設で Temporal の節ごと撤去。フィットの検証は `--seriespanel-selftest` が series 層で引き継ぐ) | ~~`--frame-lin-selftest tools/testdata` F10/F11~~ | (記録として保持) | (記録として保持) | 引退 |
