...
**表示ガンマ**は、表示レンジで正規化した値 `x` に `x^(1/γ)` を適用する表示設定。
画素ファイルの符号化方式や、測定値そのものを指す「ガンマ」ではない。
値域は 0 より大きい有限な float で、1.0（線形）と 2.2 は操作を短くするための
プリセットにすぎない。それ以外の有効値は **Custom** と表示し、近いプリセットへ
丸めない。prefs.txt、settings.jsonc、.vsession は同じ値域を保存・復元する。
