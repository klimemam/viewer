...
browse_state.h)と、その viewer 側の継ぎ目(core/main.cpp の `g_browseHost` 表、
core/ui/menus.inc の述語群)。
状態: 提案。**この文書はコードを1行も動かさない。動かすのは §5 の各段で、
それぞれ別の作業。** 前提文書: docs/background/browse/browse-inventory.md(裁定の分母)、
docs/features/browse/browse-topbar-design.md §10(実装方針の記録)、docs/background/project/split-plan.md §3/§9
(持ち出し単位と判断1)、docs/features/adapters/input-adapters.md §3.6.4(述語の分離)、
docs/features/remote/remote-headerless-design.md §5(直近で継ぎ目が動いた実例)。

語彙は既存文書のまま: 継ぎ目 / 戸 / 述語 / 召喚・降格・統合・廃止 /
持ち出し単位 / 選択バー。新語は2つだけ、理由と共に §3 / §4 で導入する:
**リンク時契約**(継ぎ目を「ホストが定義すべき関数宣言の集合」として置く形。
「表 + 名前呼び」の2本立てを1本にする言葉が既存文書に無い)と
**linktest**(スタブホストだけで browse をリンクする検査対象。「持ち出せる形」
を口約束でなくビルドの緑赤にする装置の名前が要る)。

---

## 1. 残り10点 — 数え方と勘定

**数え方を先に書く。** 分母は作らない(「90点」はユーザーの体感であり、
100 マスの表を発明すると数字が主張を持ち始める)。分子だけ数える:
...
| 10 | **Watch が名乗らない。** #12 の裁定は `…` に「最終確認 hh:mm」併記。どこにも出ない。ただし watch-design §5 は「一覧は黙って更新してよい」を原則に置く — 裁定同士が衝突している | 棚卸し #12/D6 ↔ watch-design §5 | 表示なし | (a) | **v1 では断る**(沈黙の原則の側を採る)。再訪条件: 「更新されているか分からない」という報告 |

**点数の外(構造)**: §10.8 のメニューのテーブル化と command palette。点1の
恒久解の半分だが、Browse 単独の課題ではない(メニューバー全体)。
残課題【Fable】として板に置く。段1はテーブル化を**待たない**(戸を増やすだけで
テーブルと矛盾しない)。
...
- **形態: ソース。** ヘッダ+実装の組を add_subdirectory / FetchContent で
  ...
  結合するので、バイナリ境界は嘘になる。ヘッダオンリーも**しない**: worker
...
- **repo 化の時期: split-plan §9 判断1を維持 — 今はしない。**
  ...
  FetchContent か)はその日のユーザー裁定** — 今決めても腐るだけの2択。
...
**経過措置**(段5より前に新しい戸が来た場合のみ): 表の初期化を constexpr
named-assignment builder(`constexpr BrowseHost makeHost(){ BrowseHost h;
h.toast=&toast; …; return h; }`)に変えて位置事故を即死化する。次の戸が
...
- **一覧の背景**(行の無い領域)**右クリック = `…` と同じ panel menu。**
  ファイルマネージャで最も探られる場所に同じ戸を置く。戸を**増やす**のであって
  `…` は動かさない(覚えた人の場所を変えない。ラベル付けもしない —
  ラベルは点1の原因ではなく、探した場所に戸が無かったのが原因)。
- **非既定チップ(flat / tree / a-z)のクリック = panel menu を開く。**
  チップは設定の名乗りなので、押せば設定の住処へ。今は SmallButton の見た目で
...
- 動詞は行右クリックメニューと**同一コード**(共通関数に括る。二重登録は
  必ず腐る — §10.8 がメニューについて言うことの縮小版)。ラベルも同文。
...
- 出没するが**下端**なので上の行は動かない(出没帯の禁は「一覧の上」の話。
  ...
  直後だけで、因果が画面上で読める。
- Esc の段は不変: 選択解除でバーも消える。`[x]` は同じことの可視の戸。
...
- 「connect」語彙はパネルの顔から消える(D5)。ssh の戸の文言は
...
- 継ぎ目の最終形は**リンク時契約1本**(表は段5で解消。それまで新しい戸が
...
- 点9(スピナー)・点10(Watch の名乗り): §1 の再訪条件。
