...
- 4次元より深いものは**先頭を黙って取る**
- 1次元は「高さ1の画像」になる (docs/features/adapters/npz-design.md の欠陥)
...
依存で、実測は `docs/features/media/media-support.md` §1、ライセンスは THIRD-PARTY-NOTICES.md。
...
議論と実測は [video-support.md](../media/video-support.md)。ここには結論だけ置く。
...
`unitForAnalysisKey` でそれを unit 列へ展開します([analyzers.md](../../reference/analyzers.md))。
...
- typed Reader の転置宣言は `Frame → CHW`、`Stack → FCHW` だけ。Python API が受ける
...
(docs/features/adapters/npz-design.md) と同じ土台に乗る。二つは競合ではなく、後者が前者を運ぶ。
...
両者が混ざることはない。`__viewer.npy` が存在した時点で container の戸に固定し、
marker が壊れている、0-D `i4` でない、または重複している場合も ordinary npz へ
fallback せず、生成物全体を名指しで拒否する。
...
### 4.11.2 carrier generation — Viewer-NPZ と枠付きストリーム

Viewer-NPZ (`__viewer`) と、Readerの内部transport/cacheである枠付きストリーム
(`VIEWERSTREAM`) は同じnode宣言とcarrier generation規則を使う。zip/npy memberと
行header + 宣言順blobという包みは異なるが、版が約束するmaterialize semanticsは同じである。

**1 と 5 について:** v1 には layer / layout 宣言があり、v2 は AnalysisSet を追加したが、
これは未リリースだった内部草案の履歴であって、現行 reader の互換範囲ではない。
**v3 は typed semantics を materialize 境界まで正しく適用する唯一の現行世代**である。
現行 harness は、結果がたまたま native axes / set-free でも `__viewer 3` /
`VIEWERSTREAM 3` を常に先頭へ書く。現行 reader も container / stream とも
**version が3と等しい場合だけ**読み、v1 / v2 / v4以上は木やpixelsを一つも作る前に
生成物全体を拒否する。版は個々のfeature bitではなくcarrier全体の契約だからである。
v1/v2草案と、当時の「setを含む生成物だけ2」という規則は
[reader-analysisset.md](reader-analysisset.md) §4に非規範の履歴として記録する。

**後続裁定 (2026-09-08, [Issue #242](https://github.com/klimemam/viewer/issues/242)、案B、CLOSED):**
phase④の生成物は未リリースで、旧viewerとの後方互換は要求しない。そのため上記の
exact-v3 read/writeを採用し、native-only / set-freeの新規生成物もpre-v3 readerでは
開けず、v1/v2草案も現行readerでは開けない互換コストを明示的に受け入れた。
carrier generation 3は`__viewer` / `VIEWERSTREAM`のmaterialize契約、remote protocol 15は
CHW/FCHW等をMETA/TILE/MEASUREで運ぶ能力の版であり、独立した2つの門である。
...
#### 4.12.1 session は reader を名指しする — 走らせはしない (2026-08-14 裁定 C, #179)

記憶 (memo) は**このマシンのこのユーザー自身の選択**の記録で、session は
**旅をする文書**。同僚から受け取った `.vsession` が Python ファイルを名指しして
...
- 共有時: `remoteKey`を持つkeyed materialisationは`srcShareable`のpath/member registryへ
  載せない。同じpath/member/readでもReader codeやpeerのscan epochが違えば別の画素なので、
  resident copyの共有を見送って別keyの生成物を誤ってadoptしないことを優先する
...
- **Browse の行の右クリックに「Open with reader...」** (2026-08-17 追加)。
  上の2つは「viewer が既に失敗した」ことが前提で、そのため**戸が2つ欠けていた** ——
  一度 reader に紐付いたファイルは次から memo が走るので (§4.12) 別の reader へ
  変える入口が無く、native で開ける `.npy` 等には**あえて reader を当てる**入口が
  そもそも無かった。**ファイル行にだけ**出す (reader は1パスを受け取るので、
  フォルダ行・グループ行には出さない —— §4.1)。**local 行でも remote 行でも同じ項目**
  (#180 stage 1〜2 以降 reader は peer 側で走る)。peer が受けるかどうかは peer の
  答えで、`--serve-readers` の拒否は Reader パネルが Load のときに言う (§4.13.1) ——
  Browse 側では言い直さない
...
- **開き方**: 読めないファイルをダブルクリック / 拒否メッセージの「reader を選ぶ…」/
  Inspector の reader 欄 / **Browse の行の右クリック「Open with reader...」**
  (2026-08-17 追加、§4.13)。**入口は4つ、行き先は1つ**
...
- **Watch と噛み合う。** 監視対象は**元ファイル**、変化したら adapter を通して
  読み直す (docs/features/watch/watch-design.md の再読込がそのまま使える)
