...
> **実装済み。** 本書は A/B 統計の要求と設計根拠を保存する。現行の実装状況と
> N slot への一般化は [compare-n.md](compare-n.md) §11〜§12、回帰試験は
> `selftest.abstats` / `selftest.abeq` / `selftest.export-tsv` を正典とする。

以下の「現状」は設計開始時点の記録である。当時、5つの統計パネルは `cur()` だけを
参照し、キャッシュも1枚分しか保持していなかった。本書は「重ねて表示する」と
「画像と同じ順序で横に並べる」の両方を実現するための仕様を記録する。
...
- **実測** —— f424dae(2026-07-28)で上の実験を実施して throttle を追加、2026-08-05 に
  ...
  出した状態)にならない。
- **既定が正直になった (2026-08-10)** —— 上の `--bench-panels` は
  `--bench` の**既定**になり、実行は自分が測ったパネルを名乗り、開いたのに描かれなかった
  パネルがあれば**非ゼロで落ちる** (`bench: panels measured ...` / `benchcov: ... PASS|FAIL`、
  `selftest.benchcov`)。同一マシン・同一フィクスチャで**旧既定と新既定を並べて**測った
  (`--bench 300 --bench-step`、3 回の median):

  | 実行 | 旧既定 (タブ、実際に描かれたのは Histogram 1枚) | 新既定 (3枚横並び) |
  |---|---|---|
  | compare off | 2.50 / 2.34 / 2.29 → **2.38 ms** | 12.58 / 12.35 / 12.81 → **12.58 ms** |
  | CmpSplit+follow (throttle あり) | 2.44 / 2.40 / 2.42 → **2.42 ms** | 13.13 / 12.85 / 13.15 → **13.04 ms** |
  | `--no-ab-throttle` | 4.43 / 4.25 / 4.27 → **4.32 ms** | 24.60 / 24.67 / 24.61 → **24.63 ms** |

  **8 ms 閾値への影響**: この節が定めた判定は「B 込みの draw が compare off より 8 ms 以上
  重いなら throttle」。新既定では **+12.05 ms** で閾値を超え、throttle は**維持** ——
  2026-08-05 の再測定 (12.31 / 12.75 / 24.57) と誤差内で一致する。一方、**旧既定の数字で
  同じ判定をすると +1.94 ms で閾値を下回り、throttle は不要という逆の結論になる**。
  つまり throttle の根拠は無事だが、それは再測定が `--bench-panels` を明示していたから
  であって、既定で測っていれば判定は反転していた。閾値も throttle もここでは動かさない。

  なお板の行は「Projection 単体の数字」と記していたが、測って確かめると残るタブは
  **Histogram** だった (`bench: OPEN BUT NEVER DRAWN: Temporal, Projection`)。
  どのタブが残るかはドックノードの選択状態次第で、測定が持ってはいけない性質そのもの。
