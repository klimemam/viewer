...
すべての数値列はヘッダに単位を持つ。以下の `[DN]` ヘッダは**整数入力の契約と例**である。
float 入力は単位を仮定せず、未宣言なら無単位で出す。現行 Temporal / ROIs / export が
float にも `[DN]` を付ける残差は phase④ で修正する (`sigma_col [%]` のような比率は別)。
...
ボタンは Temporal と同じ **`Copy (TSV)` / `Save (CSV)...`** の2つ、既定名
`roi_stats.csv`。

### 10.1 この表だけが持つもの — 「どこを測ったか」
...
### 10.2 座標規約 — 確認した実装の規約であって、選んだ規約ではない

**左上原点・0 始まり・x は右、y は下(y=0 が画像の最上行)。`w`/`h` は画素数で、
領域は `x..x+w-1` × `y..y+h-1`(半開区間 `[x, x+w) × [y, y+h)`)。**

これは決めたのではなく**コードから確認した**もので、根拠は3つ:

- `roiBasicStatsUncached` が `ax = rx + x` (`x < rw`)、`ay = ry + y` (`y < rh`) で
  `px()[(ay * w + ax) * ch]` を読む — 上端左端を含み、`x+w` / `y+h` は含まない。
- `roiSideCovers` の判定が `a.x + a.w <= d.w` — 右端は排他。
...
**2つ**。1つの矩形にしないのが §2 の決定そのものである —— ROI 行は**母集団**
(mean / std / min / max / n)、POI 行は**値**であり、畳めば pin の行に空欄が5列
並ぶ。

1. `# == ROI statistics ==` — 行 = side × 領域 × plane

       side  roi  x  y  w  h  ch  fold  mean [DN]  std [DN]  std / mean [%]  min [DN]  max [DN]  n  stride  note

2. `# == POI values ==` — 行 = side × pin × その面の channel

       side  poi  x  y  ch  value [DN]  note
...
- **side-major 固定**(§3.2 と同じ)。side 内は **All 行 → 置いた順**
  (`app.anns` の順序 = 画面の行順序。issue #67 が挙げた2案は同じものである)。
- ROI は**共通**(issue #76)。矩形を含まない side は `roiBasicStats` が
  クランプして交差部分の「もっともらしい数」を返してしまうので、**数値は全部 `-`、
  理由は同じ行の `note` 列**に置く。画面の「セルは `-`、理由はフッタ」を、行に
  たたんだ形である。

### 10.5 検証 — `--roi-export-selftest`

windowless・解析フィクスチャ(`--roistats-selftest` と同じ理由。加えて
`buildRoiExport` は1フレームも描かない)。期待値は全部フィクスチャから手で導出し、
