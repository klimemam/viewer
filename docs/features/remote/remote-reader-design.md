...
> 現行writerはcontainer / streamのcarrier generation 3を常時出力し、現行readerも
> version 3だけを受け入れる。v1/v2は未リリースの内部草案、v4以上は未対応の将来版として、
> tree/pixelsの前に全体拒否する。旧viewerで新規v3を開けず、現行viewerでv1/v2草案を
> 開けない互換コストを受け入れた。これはremote wire protocol 15のtyped-axes能力とは
> 別の版門である。
...
| 5 | #44 との接続 | peer 上で `run_adapter.py --stream → ファイル` (#44 で凍結した枠付き骨格は v1、0.268 s/151 MB)。v1/v2は未リリースの内部草案であり、現行writerはtyped-semantics世代 **v3** を常に出し、現行readerもv3だけを読む。**キャッシュファイルをそのまま転送元に使う**: `.vstream` の blob は C-order 連続なので、`openRaw` (serve.cpp :568) と同じ「offset + stride の ServedFile」で `readNpyRegion` が無改造で読む。キャッシュ鍵の mtime/size は **peer の stat** (§5) |
...
| 運搬形式 | 枠付きストリームの骨格は #44 レビューで**凍結済み** (`adapter-transport-review.md` §1.2)。`_esc`/LE 固定 (A2/A4) 込み。v1/v2は未リリース草案の履歴で、現行writer / readerはexact-v3 |
...
`VIEWERSTREAM 1`、AnalysisSet草案は2だった。layer/layout自体はv1から存在したが、
v1/v2はいずれも未リリースの内部草案である。2026-09-08のIssue #242ユーザー裁定B後は、
現行writerが常に3を名乗り、現行peer/clientもversion 3だけを読む。v1/v2/v4以上は
木やpixelsを一つも作る前に生成物全体を拒否する。peer は
これをキャッシュファイルとして受け、
...
| 断るもの | 文と理由 | 再訪条件 |
|---|---|---|
| **set 入りの返り値** (`VIEWERSTREAM 3`; v2は未リリース草案) | **現行も名指しで拒否する。** local carrier は役割解決・sessionを含めて読めるが、remote Reader は client 側の Ref / role-ref 名前空間を登録前に束縛できないため、返り値全体を拒否する | role-ref registry を remote identity へ拡張する新たな裁定。現行契約は input-adapters §4.11.1 / reader-analysisset §2.3・§4 |
...
| 転送方式はファイル渡しの枠付き骨格（現行 carrier は exact v3）、peer でも同じ | #45 W1/W2 の実装時に常駐ワーカを再測定 (§5.4) |
...
- container の予約 member は「小さい」(word・数値・ベクトル — loader_npz.inc
  ...
  それが渡らないための本設計である。
  carrier は exact-v3 gate とし、1 / 2 / 4 以上は client の既存文で全体拒否する。
...
- **復元**: src->path = url、src->member = 配列名 (npzOpenMember :769 と同じフィールドが
  ...
  (native であり、走るものが無い)。
- **共有境界**: `remoteKey`を持つkeyed materialisationは`srcShareable`のpath/member
  registryへ載せない。Reader codeやpeerのscan epochが異なる別keyの画素を、同じ
  path/member/readというだけでadoptしないためで、resident copyの共有よりidentityを優先する。
...
#230 phase④ の typed CHW/FCHW 境界で VERSION 15 となった。carrierは現行v3のままでも、
空layout / canonical nativeはprotocol 14 wireで同じ画素を返せる。ただしtyped
`Stack(F,H,W)`で`W <= 4`はv14のnative shape推測ならHWC Frameに化けるため、layoutが
空でもtyped axesとしてv15を要求する。
named layout、非native keyed axes、この narrow Stack 例外は client が pre-15 peer へ
META / TILE / MEASURE を送る前、かつclientがdocumentを作る前に拒否する。現行番号は
`core/remote_proto.h::VERSION` と冒頭の実装帳簿を正とする。

carrier世代は別のgateである。[Issue #242](https://github.com/klimemam/viewer/issues/242)は
2026-09-08のユーザー裁定B (CLOSED) により、現writerが`VIEWERSTREAM 3`を常に出し、
現readerもversion 3だけを受け入れると確定した。v1/v2は未リリースの内部草案、v4以上は
未対応の将来版として、木やpixelsを一つも作る前に全体拒否する。phase④生成物は未リリース
なので、旧viewerでnative-onlyを含む新規v3を開けず、現行viewerでv1/v2草案を開けない
互換コストを明示的に受け入れた。`VIEWER_SERVE_PROTOCOL=14` selftest seamはwire番号と
wire能力だけを模倣し、carrier parserの上限を2へ戻さない。したがってcanonical v3は
protocol 14 wireでも通り得る。
...
canonical native v3 Readerは通し、NPZのcanonical v3もwire上は同じ扱いとする。
