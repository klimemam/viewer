...
## 11.5 着地後の帳簿 (2026-08-13)

**段2a〜2e すべて main。** 設計の決定は1つも覆っていない。実装が足したのは
2点だけで、どちらも試験が見つけた:

- **`browse.grouped` は列挙で、bool ではない。** キーは `grouped` で
  フィールドは `rbFlat` なので、チェックボックスは**自分の名前に対して反転**
  する。W2 (「読める行は全部 widget 種を持つ」) が捕まえた —— §3.2 が
  「ウィジェットは applier が受ける型から機械的に決まる」と書いた所は、
  **型だけでは足りず名前の向きも要る**という訂正である。
- **`baseline()` は新しい3フィールドも戻す。** R8 が置いた
  `memoryBudgetGB = 8` が T3 まで生き延び、テンプレートが7つ目のキーを運んで
  いた。共有状態の罠が**1つのテストファイルの中**で起きた形で、直す場所は
  「全フィールドが組み込み既定」という baseline の定義そのもの。

| 段 | PR | 試験 |
|---|---|---|
| 2a 出所台帳 | #192 | O1..O4 |
| 2b-i 行モデル | #193 | W1..W4 |
| 2b-ii パネル | #194 | (2b-i が主張済み) |
| 2c コピー | #195 | W5 (往復) |
| 2d/2e measuring + watch | #196 | M1..M3、W1 の数が 26/33 へ |

**照合勘定は閉じた**: 編集可 26 / まだ 5 / ここではない 2 = **33行**。§3.1 が
段2d/2e 完了後として予告した数と一致する。

**v1 で断ったもの** (§11 と併せて再訪の鍵): settings.jsonc の外科的書き換え、
