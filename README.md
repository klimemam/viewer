...
- **複数 ROI** — R ツールでドラッグ作成、掴んで移動、角ハンドルでリサイズ。
  選択中の ROI に **`X` = 全幅(行バンド)/ `Y` = 全高(列バンド)のトグル**——
  もう一度押すと元の矩形に戻る。ズームアウトせずに行選択・列選択ができる。
  解析プラグインが**全 ROI を列とする比較テーブル**を出力。
...
- **プラグイン** — `plugins/` の共有ライブラリを起動時に読み込み(C ABI, [include/ps/ps_plugin.h](include/ps/ps_plugin.h))。
  解析系は `カテゴリ/測定名` で分類され、**Measure メニュー**(前提条件ヒント付き)または
  Analysis パネルの2段セレクタから実行(詳細: [docs/analyzers.md](docs/analyzers.md)):
  - `stats/moments` — ch毎 mean/var/std/min/max/percentile/エントロピー
