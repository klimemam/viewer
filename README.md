...
| 📐 **複数 ROI・複数 POI** | ツールモード(Nav/ROI/Pin)、`X`/`Y` で行/列バンドのトグル、選択 ROI から**動的クロップ⇔復元** |
| 📊 **その場で測る** | 内蔵ヒストグラム(CFA 4ch 分離・ROI 追従)+ 解析プラグイン: 統計 / ノイズフロア / **PRNU・FPN** / シャープネス / **ISO 12233 e-SFR(カーブ表示)**。結果は **ROI × 指標の比較グリッド** |
