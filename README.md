...
| 🆚 **A/B 比較** | `\` で **ワイプ(真ん中の仕切りをドラッグ)⇔ 並べて表示**。ズーム/位置は常に共通、Inspector に **A / B / A−B** の画素値。`B` 長押しで B 全面、`Shift+B` で**同じ塊のフレーム同士**を比較 |
...
| 行・列バンド | ROI を選択して `X`(全幅)`Y`(全高)——再押下で復元 |
...
| A/B 比較(ワイプ⇔並べて)/ A⇄B 入替 | `\` or `C` / `Shift+\` |
